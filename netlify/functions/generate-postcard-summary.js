  ...
  try {
    ...
    if (!date) {
      return {
        statusCode: 400,
        headers,
        body: JSON.stringify({ error: 'Datum is vereist' })
      };
    }
    ...
    if (data?.dutch_films?.length > 0) {
      dataPoints.push(`🎬 Nederlandse films: ${data.dutch_films.map((film) => film.title).join(', ')}`);
    }
    if (data?.movies?.length > 0) {
      dataPoints.push(`🎭 Internationale films: ${data.movies.map((movie) => movie.title).join(', ')}`);
    }
    ...
    return {
      statusCode: 500,
      headers,
      body: JSON.stringify({ error: 'Er is een fout opgetreden bij het genereren van de samenvatting' })
    };
  }
};
