...
exports.handler = async (event, context) => {
  ...
  try {
    const { date, event, name, historicalData, userId, userEmail } = JSON.parse(event.body);

    if (!date || !event || !name) {
      return {
        statusCode: 400,
        headers,
        body: JSON.stringify({ error: 'Missing required fields: date, event, name' })
      };
    }
...
async function generateScriptWithGemini(date, event, name, dataSummary) {
  try {
    // Check if Gemini API key is available
    const hasValidApiKey = process.env.GEMINI_API_KEY && 
                          process.env.GEMINI_API_KEY.trim() !== '' && 
...
async function generateAudioWithGeminiTTS(script) {
  try {
    // Check if Gemini API key is available
    const hasValidApiKey = process.env.GEMINI_API_KEY && 
                          process.env.GEMINI_API_KEY.trim() !== '' && 
...
function prepareDataForPodcast(historicalData) {
  ...
  try {
    ...
    if (historicalData.tv_series && historicalData.tv_series.length > 0) {
      ...
      const seriesDetails = series.map(s => {
    ...
    if (historicalData.news && historicalData.news.length > 0) {
      ...
      const newsSample = historicalData.news.slice(0, 4).map(n => {
    ...
    if (historicalData.music && historicalData.music.length > 0) {
      ...
      const musicDetails = music.map(m => {
    ...
    if (historicalData.science && historicalData.science.length > 0) {
      ...
      const scienceDetails = science.map(s => {
    ...
    if (historicalData.business && historicalData.business.length > 0) {
      ...
      const businessDetails = business.map(b => {
    ...
    if (historicalData.politics && historicalData.politics.length > 0) {
      ...
      const politicsDetails = politics.map(p => {
...
async function logPodcastGeneration(userId, date, event) {
